...
class EEGTaskProcessor:
    def __init__(self, raw, events, task_name):
        self.raw = raw
        self.events = events
    ...
    def get_filtered(self, l_freq, h_freq):
        key = (l_freq, h_freq)
    ...
    def _sus_preprocess(self, l_freq, h_freq, tmin=0.0, duration=2.4):
        filtered = self.get_filtered(l_freq, h_freq)
        ...
        tmax = tmin + duration
        epochs = Epochs(filtered, events=events_array, event_id=event_id,
                        tmin=tmin, tmax=tmax, baseline=None, proj=True,
                        preload=True, detrend=1)
        labels = stim_rows['label'].values[epochs.selection]
        return epochs, labels

    def _resting_preprocess(self, l_freq, h_freq, tmin=0.0, tmax=20.0):
        filtered = self.get_filtered(l_freq, h_freq)
        df = self.events
        ...
        epochs = Epochs(filtered, events, event_id=eye_map, tmin=tmin, tmax=tmax,
